# (Swift 5, iOS 16): MVVM, Combine, IUKit and XCTest.
...
• .finished: The game has ended and it is time to update the Round.Match and Team models. And then, it must recalculate the teams standings to update the Standings board.
...
### The players must have a head2head battle based on their positions.
...
If the Team committed a foul, then it loses the ball to the other team, and must fall back to a defense position.
...
The more fit the players are at the start of the game, the less AthleticDecay they will have.
